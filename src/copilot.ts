...
export function renderCopilotAuth(
  ...
  root.innerHTML = `<div class="copilot-auth-card"><h3>GitHub Copilot</h3><p class="settings-hint">Supplies AI credentials only. Repository access and future comments use the separate GitHub repository connection. Sign-in requests profile access and refresh, not repository access; GitHub may retain permissions you previously granted this OAuth App.</p><p role="status">Reading Copilot accounts...</p><p class="copilot-error" role="alert" hidden></p><div class="copilot-flow"></div><div class="copilot-accounts"></div><p class="settings-hint">A check means verified sign-in, not a subscription, seat or model test. Connections save immediately in macOS Keychain, independently of Save changes. Disconnect removes this role's local credential, not your GitHub authorization or repository connection.</p></div>`;
