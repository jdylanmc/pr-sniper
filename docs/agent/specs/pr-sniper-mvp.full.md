...
The application runs monitoring only while its tray process is active, with opt-in launch at login. GitHub and the selected local agent remain external dependencies with independent authentication and limits. The registered GitHub OAuth App uses application ID `3878184`, public client ID `Ov23lidoL3QovWyfxnA4`, Authorization Code + S256 PKCE in the default browser, exact callback `http://127.0.0.1:53682/oauth/github/callback`, expiring tokens, no device flow and no client secret. It requests GitHub's broad `repo` scope because GitHub exposes no read-only OAuth scope for arbitrary private repositories; Settings must state that consent implication before sign-in. Each repository configuration names one provider, stable provider-account identity and stable provider-repository identity. When multiple accounts can access the same repository, the user explicitly chooses the binding; account removal leaves affected repositories visibly unbound or disabled rather than transferring them.
...
- The PR Sniper GitHub OAuth App is separately registered under `jdylanmc` as application ID `3878184`, with public client ID `Ov23lidoL3QovWyfxnA4`, exact fixed loopback callback, expiring tokens enabled, device flow disabled and no client secret.
- GitHub OAuth App Authorization Code + S256 PKCE supports a native public client without an embedded client secret. User access tokens rotate with refresh tokens, so persistence must replace the pair together and serialize competing refresh attempts.
- `GET /user/repos` with owner, collaborator and organization-member affiliations plus all visibility discovers repositories accessible to the user token. Selection remains explicit; discovery never enables monitoring automatically.
...
- PR-010 [AC-007]: The MVP shall connect through the registered PR Sniper GitHub OAuth App using Authorization Code + S256 PKCE and `scope=repo` in the default system browser without requiring GitHub CLI, bind the exact localhost callback before opening the browser, strictly correlate and consume state once, bound cancellation and timeout, validate `/user`, and require explicit stable-identity confirmation before persistence.
...
- PR-046 [AC-007, AC-018]: Settings shall distinguish disconnected, browser-connecting, pending identity confirmation, connected stable identity, failed and reconnect-required states, keep secure-storage confirmation failures visibly retryable, and never imply repository selection or enable review execution, publication, notifications or merging.
...
- PD-003 [AC-007]: GitHub is the first provider and the registered PR Sniper GitHub OAuth App's Authorization Code + S256 PKCE flow with broad `repo` scope in the default system browser is the product sign-in. GitHub CLI is optional development or migration context only.
...
| AC-007         | PR-010–PR-011, PR-046, PR-048, PR-054, PD-003, PD-012, PD-014 | GitHub OAuth App registration, browser OAuth with PKCE and provider extension contract |
