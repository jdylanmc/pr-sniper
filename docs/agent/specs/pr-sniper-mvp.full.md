...
The application runs monitoring only while its tray process is active, with opt-in launch at login. GitHub and the selected local agent remain external dependencies with independent authentication and limits. The registered GitHub OAuth App uses application ID `3878184`, public client ID `Ov23lidoL3QovWyfxnA4`, Authorization Code + S256 PKCE in the default browser, expiring tokens, no device flow and no client secret. It requests GitHub's broad `repo` scope because GitHub exposes no read-only OAuth scope for arbitrary private repositories; Settings must state that consent implication before sign-in. The fixed callback `http://127.0.0.1:53682/oauth/github/callback` is accepted for the #35 proof, but #36 must replace it with a registered PR Sniper application URI before MVP completion.
...
- PR-010 [AC-007]: The MVP shall connect through the registered PR Sniper GitHub OAuth App using Authorization Code + S256 PKCE and `scope=repo` in the default system browser without requiring GitHub CLI, return through a registered PR Sniper application callback, strictly correlate and consume state once, bound cancellation and timeout, validate `/user`, and require explicit stable-identity confirmation before persistence. The fixed localhost callback remains a proof-stage implementation only until #36.
