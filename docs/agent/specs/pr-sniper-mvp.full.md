...
On 2026-09-25 the human approved end-to-end Copilot Settings with app-owned
native sign-in, independently configurable AI accounts and explicit real-model
selection per reusable Agent. This scoped amendment supersedes PR-015's former
agent-tool-owned-only authentication rule. It does not change review trust,
automation/publication gates or the no-provider-approval boundary, and does not
...
- PR-004 [AC-003]: Global defaults and per-repository overrides shall cover fixed interval or five-field cron schedule with explicit time zone, watched GitHub identities, reviewer-assignment trigger, agent adapter, model or named-agent selector, prompt, automatic agent start, and automatic comment publication. A reusable Agent shall explicitly select a stable AI provider/account reference (`AiAccount`) and an actual model returned for that account, independently of its repository assignments and the repository acting account. Copilot shall be represented as the provider, not a model. No example identity, first account or model shall become an implicit default or fallback; direct Claude/Codex/Grok integrations remain unavailable, without excluding Claude models supplied through Copilot.
...
- PR-015 [AC-003, AC-009, AC-018]: PR Sniper shall own Copilot's native secretless sign-in, explicit verified stable-identity confirmation, OS-secured credentials, refresh and independent account lifecycle. Repository connections shall supply repository-access/publication credentials, while the Copilot connection supplies AI credentials; logical role separation does not by itself establish independent upstream OAuth grants. The adapter shall explicitly pin account-specific SDK/runtime identity and model enumeration without ambient terminal, stored first-party login, direct-provider or runtime-configuration fallback. A green check shall verify sign-in only: no seat, subscription or inference probe is required or permitted by that check. Model availability/errors shall remain separate, visible and retryable. Disconnect shall retain dependent Agents and assignments as blocked, and legacy or unavailable saved selections shall remain loadable until explicitly reconfigured, not deleted or silently remapped. Sign-in/account/model Settings shall not execute reviews or publication. The adapter contract shall still permit later local agents without changing provider or monitoring domain models; no authentication mechanism for an unimplemented provider is selected here.
...
- PR-051 [AC-002, AC-018]: Removing a repository-provider account shall remove only that account's credentials and registry entry, clear its active selection if applicable, retain every other account, and leave repositories bound to the removed account visibly blocked until an explicit rebind. Copilot AI disconnect shall instead retain its stable identity and dependent Agent references for reconnect while removing that role's usable credentials. PR Sniper shall not silently select another account in either role.
