...
- AC-002: The user can add, edit, disable, re-enable, and remove GitHub repositories without an application-defined maximum; provider and local-resource limits are surfaced explicitly.
- AC-003: The user can configure global defaults and per-repository overrides for schedule, watched authors, reviewer-assignment trigger, local agent, model or agent selector, prompt, automatic agent start, and automatic comment publication.
