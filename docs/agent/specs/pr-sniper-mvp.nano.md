...
- AC-002: The user can concurrently retain multiple GitHub accounts and add, edit, disable, re-enable, and remove GitHub repositories without an application-defined maximum; each repository is bound to one provider and stable provider-account identity, overlapping access requires an explicit account choice, and provider or local-resource limits are surfaced explicitly.
...
- AC-007: The user connects one or more accounts through the PR Sniper GitHub App with Authorization Code + S256 PKCE in the default system browser, a fixed localhost loopback callback, and the public client ID. PR Sniper validates the returned stable identity and requires explicit confirmation before persisting account-addressed credentials; bind, browser-open, callback, cancellation, timeout, provider, network, wrong-identity and secure-storage failures remain explicit. GitHub CLI may remain only as an identified development or migration probe, not product sign-in. The account and credential contract is provider-neutral, while GitHub is the only implemented MVP provider.
