...
- AC-007: The user connects one or more accounts through the PR Sniper GitHub OAuth App with Authorization Code + S256 PKCE in the default system browser, fixed callback `http://127.0.0.1:53682/oauth/github/callback`, public client ID `Ov23lidoL3QovWyfxnA4`, no client secret, and the broad `repo` scope required for arbitrary private repositories. PR Sniper makes that consent tradeoff explicit, validates `/user`, and requires explicit identity confirmation before persisting account-addressed credentials; bind, browser-open, callback, cancellation, timeout, missing/revoked scope, organization policy, provider, network, wrong-identity and secure-storage failures remain explicit. GitHub CLI may remain only as an identified development or migration probe, not product sign-in.
