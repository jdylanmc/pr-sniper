...
native AI sign-in, independently configurable AI accounts, and an explicit
account-returned model per reusable Agent. It supersedes the earlier
agent-tool-owned-only authentication rule, not the execution, trust,
publication or approval boundaries. Historical Discovery remains unchanged;
these requirements are not evidence of live provider acceptance.
...
- AC-009: The MVP includes a GitHub Copilot CLI review-agent adapter and an adapter contract that supports later local agents without changing the monitoring or provider domain model. PR Sniper owns Copilot's secretless native sign-in and secure account lifecycle, verifies stable GitHub identity and requires confirmation before persistence, and pins each account's SDK/runtime context without ambient credential fallback. A green sign-in check verifies identity only, not subscription, seat, model availability or inference. Model errors remain separate and retryable; AI disconnect retains dependent Agents and assignments as blocked until explicit reconnect or reselection. Legacy unconfigured Agents remain loadable without automatic remapping. These Settings operations do not authorize or execute reviews or publication.
